<?php
/**
 * EGroupware SiteMgr CMS - Content storage object
 *
 ...
 * - rewritten with the new db-functions and documented by RalfBecker-AT-outdoor-training.de
 * - new data get JSON encoded, existing PHP serialized data can still be read
 */
class Content_SO
{
	/**
	...
	*/
	 ...
	 * @param string $_lang 2-char language id
	 * @return array of block-objects
	 */
	function getallblocksforarea($area,$cat_list,$page_id,$_lang)
	{
		$lang = $this->db->quote($_lang);
	...
	*/
	 ...
	 * @return array
	 */
	private function merge_arguments_lang($_args,$args_lang)
	{
		$args = (array) json_php_unserialize($_args);

		if (($arr = json_php_unserialize($args_lang)) !== false)	// is a serialized value (we dont use serialize(false)!)
		{
			$args_lang = (array) $arr;
		}
		else
	...
	*/
	function getallversionsforblock($blockid, $_lang)
	{
		$lang = $this->db->quote($_lang);
	...
	function getversion($version_id,$lang=false)
	{
		...
		if ($this->db->next_record())
		{
			 return $lang ?
				...
				json_php_unserialize($this->db->f('arguments'));
		}
		return false;
	}
	 ...
	 */
	function &getblock($block_id, $_lang)
	{
		if (!$_lang) $nolang=true;
		$lang = $this->db->quote($_lang);
	...
	function saveversiondata($block_id,$version_id,$data)
	{
		...
		return $this->db->update($this->content_table,array(
				'arguments' => json_encode($data, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),
			),array(
				'version_id' => $version_id,
				'block_id'   => $block_id,
			),__LINE__,__FILE__);
	...
	function saveversiondatalang($version_id,$data,$lang)
	{
		...
		return $this->db->insert($this->content_lang_table,array(
				'arguments_lang' => count($data) == 1 && isset($data['htmlcontent']) ?
					$data['htmlcontent'] : json_encode($data, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)
			),array(
				'version_id'  => $version_id,
				'lang'      => $lang,
			),__LINE__,__FILE__);
